...
public class JT1078Controller {
    ...
    public DeferredResult<RetResult<StreamContent>> startLive(HttpServletRequest request,
        ...
        jt1078PlayService.play(phoneNumber, channelId, type, RetResult -> {
            RetResult<StreamContent> RetResultForFinish = new RetResult<>();
            RetResultForFinish.setCode(RetResult.getCode());
            RetResultForFinish.setMsg(RetResult.getMsg());
            if (RetResult.getCode() == InviteErrorCode.SUCCESS.getCode()) {
                StreamInfo streamInfo = RetResult.getData();
                ...
                if (streamInfo != null) {
                    ...
                    RetResultForFinish.setData(new StreamContent(streamInfo));
                }
            }
            result.setResult(RetResultForFinish);
        });

        return result;
    }

    @Operation(summary = "JT-结束点播")
    @Parameter(name = "phoneNumber", description = "设备手机号", required = true)
    @Parameter(name = "channelId", description = "通道国标编号, 一般为从1开始的数字", required = true)
    @GetMapping("/live/stop")
    public RetResult<Void> stopLive(HttpServletRequest request,
                                    @Parameter(required = true) String phoneNumber,
                                    @Parameter(required = true) String channelId) {
        jt1078PlayService.stopPlay(phoneNumber, channelId);
    ...
    public RetResult<Void> stopTalk(HttpServletRequest request,
                                    @Parameter(required = true) String phoneNumber,
                                    @Parameter(required = true) String channelId) {
    ...
    @Operation(summary = "JT-暂停点播")
    @Parameter(name = "phoneNumber", description = "设备手机号", required = true)
    @Parameter(name = "channelId", description = "通道国标编号, 一般为从1开始的数字", required = true)
    @GetMapping("/live/pause")
    public RetResult<Void> pauseLive(HttpServletRequest request,
                                     @Parameter(required = true) String phoneNumber,
                                     @Parameter(required = true) String channelId) {
    ...
    @Operation(summary = "JT-继续点播")
    @Parameter(name = "phoneNumber", description = "设备手机号", required = true)
    @Parameter(name = "channelId", description = "通道国标编号, 一般为从1开始的数字", required = true)
    @GetMapping("/live/continue")
    public RetResult<Void> continueLive(HttpServletRequest request,
                                        @Parameter(required = true) String phoneNumber,
                                        @Parameter(required = true) String channelId) {
    ...
    @Operation(summary = "JT-切换码流类型")
    @Parameter(name = "phoneNumber", description = "设备手机号", required = true)
    @Parameter(name = "channelId", description = "通道国标编号, 一般为从1开始的数字", required = true)
    @Parameter(name = "streamType", description = "0:主码流; 1:子码流", required = true)
    @GetMapping("/live/switch")
    public RetResult<Void> changeStreamType(HttpServletRequest request,
                                            @Parameter(required = true) String phoneNumber,
                                            @Parameter(required = true) String channelId,
    ...
    @Operation(summary = "JT-录像-查询资源列表")
    @Parameter(name = "phoneNumber", description = "设备手机号", required = true)
    @Parameter(name = "channelId", description = "通道国标编号, 一般为从1开始的数字", required = true)
    ...
    public RetResult<List<J1205.JRecordItem>> playbackList(HttpServletRequest request,
                                                           @Parameter(required = true) String phoneNumber,
                                                           @Parameter(required = true) String channelId,
        ...
        if (recordList == null) {
            ...
            return RetResponse.makeOKRsp(recordList);
        }
    }

    @Operation(summary = "JT-录像-开始回放")
    @Parameter(name = "phoneNumber", description = "设备手机号", required = true)
    @Parameter(name = "channelId", description = "通道国标编号, 一般为从1开始的数字", required = true)
    @Parameter(name = "startTime", description = "开始时间,格式： yyyy-MM-dd HH:mm:ss", required = true)
    @Parameter(name = "endTime", description = "结束时间,格式： yyyy-MM-dd HH:mm:ss", required = true)
    ...
    public DeferredResult<RetResult<StreamContent>> recordLive(HttpServletRequest request,
                                                               @Parameter(required = true) String phoneNumber,
                                                               @Parameter(required = true) String channelId,
                                                               @Parameter(required = true) String startTime,
                                                               @Parameter(required = true) String endTime,
        ...
        result.onTimeout(() -> {
            ...
            RetResult<StreamContent> RetResult = new RetResult<>();
            RetResult.setCode(ErrorCode.ERROR100.getCode());
            RetResult.setMsg("回放超时");
            result.setResult(RetResult);
            jt1078PlayService.stopPlay(phoneNumber, channelId);
        });

        jt1078PlayService.playback(phoneNumber, channelId, startTime, endTime, type, rate, playbackType, playbackSpeed, RetResult -> {
            RetResult<StreamContent> RetResultForFinish = new RetResult<>();
            RetResultForFinish.setCode(RetResult.getCode());
            RetResultForFinish.setMsg(RetResult.getMsg());
            if (RetResult.getCode() == InviteErrorCode.SUCCESS.getCode()) {
    ...
    public RetResult<Void> recordControl(@Parameter(required = true) String phoneNumber,
                                         @Parameter(required = true) String channelId,
    ...
    @Operation(summary = "JT-录像-结束回放")
    @Parameter(name = "phoneNumber", description = "设备手机号", required = true)
    @Parameter(name = "channelId", description = "通道国标编号, 一般为从1开始的数字", required = true)
    @GetMapping("/playback/stop")
    public RetResult<Void> stopPlayback(HttpServletRequest request,
                                        @Parameter(required = true) String phoneNumber,
                                        @Parameter(required = true) String channelId) {
    ...
    @Operation(summary = "JT-录像-获取下载地址")
    @Parameter(name = "phoneNumber", description = "设备手机号", required = true)
    @Parameter(name = "channelId", description = "通道国标编号, 一般为从1开始的数字", required = true)
    @Parameter(name = "startTime", description = "开始时间,格式： yyyy-MM-dd HH:mm:ss", required = true)
    @Parameter(name = "endTime", description = "结束时间,格式： yyyy-MM-dd HH:mm:ss", required = true)
...
//        response.setContentLength(394983300);
    ...
    @GetMapping("/ptz")
    public RetResult<Void> ptz(String phoneNumber, String channelId, String command, int speed) {
    ...
    @Operation(summary = "JT-补光灯开关")
    @Parameter(name = "phoneNumber", description = "设备手机号", required = true)
    @Parameter(name = "channelId", description = "通道国标编号, 一般为从1开始的数字", required = true)
    @Parameter(name = "command", description = "控制指令,允许值: on off", required = true)
    @GetMapping("/fill-light")
    public RetResult<Void> fillLight(String phoneNumber, String channelId, String command) {
    ...
    @Operation(summary = "JT-雨刷开关")
    @Parameter(name = "phoneNumber", description = "设备手机号", required = true)
    @Parameter(name = "channelId", description = "通道国标编号, 一般为从1开始的数字", required = true)
    @Parameter(name = "command", description = "控制指令,允许值: on off", required = true)
    @GetMapping("/wiper")
    public RetResult<Void> wiper(String phoneNumber, String channelId, String command) {
    ...
    public RetResult<Void> tempPositionTrackingControl(String phoneNumber, Integer timeInterval, Long validityPeriod) {
    ...
    @Operation(summary = "JT-人工确认报警消息")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    @Parameter(name = "timeInterval", description = "时间间隔,单位为秒,时间间隔为0 时停止跟踪,停止跟踪无需带后继字段", required = true)
    @Parameter(name = "validityPeriod", description = "位置跟踪有效期, 单位为秒,终端在接收到位置跟踪控制消息后,在有效期截止时间之前依据消息中的时间间隔发送位置汇报", required = true)
    @PostMapping("/confirmation-alarm-message")
    public RetResult<Void> confirmationAlarmMessage(@RequestBody ConfirmationAlarmMessageParam param) {
    ...
    public RetResult<Integer> textMessage(@RequestBody TextMessageParam textMessageParam) {
        ...
        if (result == 0) {
            ...
            RetResult<Integer> fail = RetResponse.makeRsp(ErrorCode.ERROR100.getCode(), ErrorCode.ERROR100.getMsg());
            fail.setData(result);
            return fail;
        }
    }

    @Operation(summary = "JT-电话回拨")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    ...
    public RetResult<Integer> telephoneCallback(String phoneNumber, Integer sign, String destPhoneNumber) {
    ...
    @Operation(summary = "JT-设置电话本")
    @Parameter(name = "setPhoneBookParam", description = "设置电话本参数", required = true)
    @PostMapping("/set-phone-book")
    public RetResult<Integer> setPhoneBook(@RequestBody SetPhoneBookParam setPhoneBookParam) {
    ...
    @Operation(summary = "JT-车门控制")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    @Parameter(name = "open", description = "开启车门", required = true)
    @GetMapping("/control/door")
    public RetResult<Integer> controlDoor(String phoneNumber, Boolean open) {
        ...
        if (positionBaseInfo == null || positionBaseInfo.getStatus() == null) {
            return RetResponse.makeRsp(ErrorCode.ERROR100.getCode(), "控制失败");
        }
    ...
    @Operation(summary = "JT-更新圆形区域")
    @Parameter(name = "areaParam", description = "设置区域参数", required = true)
    @PostMapping("/area/circle/update")
    public RetResult<Integer> updateAreaForCircle(@RequestBody SetAreaParam areaParam) {
    ...
    @Operation(summary = "JT-追加圆形区域")
    @Parameter(name = "areaParam", description = "设置区域参数", required = true)
    @PostMapping("/area/circle/add")
    public RetResult<Integer> addAreaForCircle(@RequestBody SetAreaParam areaParam) {
    ...
    @Operation(summary = "JT-修改圆形区域")
    @Parameter(name = "areaParam", description = "设置区域参数", required = true)
    @PostMapping("/area/circle/edit")
    public RetResult<Integer> editAreaForCircle(@RequestBody SetAreaParam areaParam) {
    ...
    @Operation(summary = "JT-删除圆形区域")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    @Parameter(name = "ids", description = "待删除圆形区域的id，例如1,2,3", required = true)
    @GetMapping("/area/circle/delete")
    public RetResult<Integer> deleteAreaForCircle(String phoneNumber, @RequestParam(value = "ids", required = false) List<Long> ids) {
    ...
    @Operation(summary = "JT-查询圆形区域")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    @GetMapping("/area/circle/query")
    public RetResult<List<JTAreaOrRoute>> queryAreaForCircle(String phoneNumber, @RequestParam(value = "ids", required = false) List<Long> ids) {
    ...
    @Operation(summary = "JT-更新矩形区域")
    @Parameter(name = "areaParam", description = "设置区域参数", required = true)
    @PostMapping("/area/rectangle/update")
    public RetResult<Integer> updateAreaForRectangle(@RequestBody SetAreaParam areaParam) {
    ...
    @Operation(summary = "JT-追加矩形区域")
    @Parameter(name = "areaParam", description = "设置区域参数", required = true)
    @PostMapping("/area/rectangle/add")
    public RetResult<Integer> addAreaForRectangle(@RequestBody SetAreaParam areaParam) {
    ...
    @Operation(summary = "JT-修改矩形区域")
    @Parameter(name = "areaParam", description = "设置区域参数", required = true)
    @PostMapping("/area/rectangle/edit")
    public RetResult<Integer> editAreaForRectangle(@RequestBody SetAreaParam areaParam) {
    ...
    @Operation(summary = "JT-删除矩形区域")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    @Parameter(name = "ids", description = "待删除圆形区域的id，例如1,2,3", required = true)
    @GetMapping("/area/rectangle/delete")
    public RetResult<Integer> deleteAreaForRectangle(String phoneNumber, @RequestParam(value = "ids", required = false) List<Long> ids) {
    ...
    @Operation(summary = "JT-查询矩形区域")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    @GetMapping("/area/rectangle/query")
    public RetResult<List<JTAreaOrRoute>> queryAreaForRectangle(String phoneNumber, @RequestParam(value = "ids", required = false) List<Long> ids) {
    ...
    @Operation(summary = "JT-设置多边形区域")
    @Parameter(name = "areaParam", description = "设置区域参数", required = true)
    @PostMapping("/area/polygon/set")
    public RetResult<Integer> setAreaForPolygon(@RequestBody SetAreaParam areaParam) {
    ...
    @Operation(summary = "JT-删除多边形区域")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    @Parameter(name = "ids", description = "待删除圆形区域的id，例如1,2,3", required = true)
    @GetMapping("/area/polygon/delete")
    public RetResult<Integer> deleteAreaForPolygon(String phoneNumber, @RequestParam(value = "ids", required = false) List<Long> ids) {
    ...
    @Operation(summary = "JT-查询多边形区域")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    @GetMapping("/area/polygon/query")
    public RetResult<List<JTAreaOrRoute>> queryAreaForPolygon(String phoneNumber, @RequestParam(value = "ids", required = false) List<Long> ids) {
    ...
    @Operation(summary = "JT-设置路线")
    @Parameter(name = "areaParam", description = "设置区域参数", required = true)
    @PostMapping("/route/set")
    public RetResult<Integer> setRoute(@RequestBody SetAreaParam areaParam) {
    ...
    @Operation(summary = "JT-删除路线")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    @Parameter(name = "ids", description = "待删除圆形区域的id，例如1,2,3", required = true)
    @GetMapping("/route/delete")
    public RetResult<Integer> deleteRoute(String phoneNumber, @RequestParam(value = "ids", required = false) List<Long> ids) {
    ...
    @Operation(summary = "JT-查询路线")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    @GetMapping("/route/query")
    public RetResult<List<JTAreaOrRoute>> queryRoute(String phoneNumber, @RequestParam(value = "ids", required = false) List<Long> ids) {
    ...
    @Operation(summary = "JT-上报驾驶员身份信息请求")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    @GetMapping("/driver-information")
    public RetResult<JTDriverInformation> queryDriverInformation(String phoneNumber) {
        ...
        if (jtDriverInformation != null) {
            return RetResponse.makeOKRsp(jtDriverInformation);
        } else {
            return RetResponse.makeRsp(ErrorCode.ERROR100.getCode(), ErrorCode.ERROR100.getMsg());
        }
    }

    @Operation(summary = "JT-摄像头立即拍摄命令")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    @PostMapping("/shooting")
    public RetResult<List<Long>> shooting(@RequestBody ShootingParam param) {
...
//            response.addHeader("Content-Disposition", "attachment;filename=" + URLEncoder.encode(phoneNumber + "_" + channelId + ".jpg", "UTF-8"));
    ...
    @Operation(summary = "JT-存储多媒体数据检索")
    @Parameter(name = "param", description = "存储多媒体数据参数", required = true)
    @PostMapping("/media/list")
    public RetResult<List<JTMediaDataInfo>> queryMediaData(@RequestBody QueryMediaDataParam param) {
    ...
    @Operation(summary = "JT-单条存储多媒体数据上传")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    ...
    @Operation(summary = "JT-单条存储多媒体数据删除")
    @Parameter(name = "phoneNumber", description = "设备编号", required = true)
    @Parameter(name = "mediaId", description = "多媒体ID", required = true)
...
//    @Operation(summary = "JT-存储多媒体数据上传命令")
//    @Parameter(name = "param", description = "存储多媒体数据参数", required = true)
//    @PostMapping("/media-data-upload")
//    public DeferredResult<RetResult<List<String>>> updateMediaData(@RequestBody QueryMediaDataParam param){
//
//        log.info("[JT-存储多媒体数据上传命令] param: {}", param );
//        DeferredResult<RetResult<List<String>>> deferredResult = new DeferredResult<>(30000L);
//        List<String> resultList = new ArrayList<>();
//
//        deferredResult.onTimeout(()->{
//            log.info("[JT-存储多媒体数据上传命令超时] param: {}", param );
//            RetResult<List<String>> fail = RetResponse.makeRsp(ErrorCode.ERROR100.getCode(), ErrorCode.ERROR100.getMsg());
//            fail.setMsg("超时");
...
//        }
//        if (ids.isEmpty()) {
//            deferredResult.setResult(RetResponse.makeRsp(ErrorCode.ERROR100.getCode(), ErrorCode.ERROR100.getMsg()));
...
//    }
    ...
    public RetResult<Void> startRecord(HttpServletRequest request,
                                       @Parameter(required = true) String phoneNumber,
    ...
    @Operation(summary = "JT-停止录音")
    @Parameter(name = "phoneNumber", description = "设备手机号", required = true)
    @Parameter(name = "time", description = "录音时间，单位为秒(s) ,0 表示一直录音", required = false)
    @Parameter(name = "save", description = "0:实时上传；1:保存", required = false)
    @Parameter(name = "samplingRate", description = "音频采样率， 0:8K；1:11K；2:23K；3:32K", required = false)
    @GetMapping("/record/stop")
    public RetResult<Void> stopRecord(HttpServletRequest request,
                                      @Parameter(required = true) String phoneNumber,
                                      @Parameter(required = false) Integer time,
                                      @Parameter(required = false) Integer save,
                                      @Parameter(required = false) Integer samplingRate
    ...
    @Operation(summary = "JT-查询终端音视频属性")
    @Parameter(name = "phoneNumber", description = "设备手机号", required = true)
