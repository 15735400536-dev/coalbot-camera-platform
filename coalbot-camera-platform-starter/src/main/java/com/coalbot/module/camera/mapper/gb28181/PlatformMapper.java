...
public interface PlatformMapper {
    ...
    @Insert("INSERT INTO wcp_platform (enable, name, server_gb_id, server_gb_domain, server_ip, server_port,device_gb_id,device_ip,"+
    ...
    @Delete("DELETE FROM wcp_platform WHERE id=#{id}")
    int delete(@Param("id") String id);

    @Select(" <script>" +
            " SELECT pp.*, " +
            " ( (SELECT count(0) FROM wcp_platform_channel pc WHERE pc.platform_id = pp.id ) + " +
            "  (SELECT count(0) FROM wcp_platform_group pg WHERE pg.platform_id = pp.id ) * pp.catalog_with_group  + " +
            "  (SELECT count(0) FROM wcp_platform_region pr WHERE pr.platform_id = pp.id ) * pp.catalog_with_region + " +
            "  pp.catalog_with_platform " +
            "    ) as channel_count" +
            " FROM wcp_platform pp where 1=1 " +
            " <if test='query != null'> " +
    ...
    @Select("SELECT * FROM wcp_platform WHERE server_id=#{serverId} and enable=#{enable} ")
    List<Platform> queryEnableParentPlatformListByServerId(@Param("serverId") String serverId, @Param("enable") boolean enable);

    @Select("SELECT * FROM wcp_platform WHERE enable=true and as_message_channel=true")
    ...
    @Update("UPDATE wcp_platform SET status=#{online}, server_id = #{serverId} WHERE id=#{id}" )
    int updateStatus(@Param("id") String id, @Param("online") boolean online, @Param("serverId") String serverId);

    @Select("SELECT server_id FROM wcp_platform WHERE enable=true and server_id != #{serverId} group by server_id")
    List<String> queryServerIdsWithEnableAndNotInServer(@Param("serverId") String serverId);

    @Select("SELECT * FROM wcp_platform WHERE server_id = #{serverId}")
    List<Platform> queryByServerId(@Param("serverId") String serverId);

    @Select("SELECT * FROM wcp_platform ")
    List<Platform> queryAll();

    @Select("SELECT * FROM wcp_platform WHERE enable=true and server_id = #{serverId}")
    List<Platform> queryServerIdsWithEnableAndServer(@Param("serverId") String serverId);

    @Update("UPDATE wcp_platform SET status=false where server_id = #{serverId}" )
