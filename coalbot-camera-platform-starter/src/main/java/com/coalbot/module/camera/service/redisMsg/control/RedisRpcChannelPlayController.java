...
import com.coalbot.module.camera.gb28181.bean.InviteMessageInfo;
...
public class RedisRpcChannelPlayController extends RpcController {
    ...
    public RedisRpcResponse playChannel(RedisRpcRequest request) {
        String channelId = request.getParam().toString();
...
//            channelPlayService.playbackResume(streamId);
    ...
    public RedisRpcResponse frontEndCommand(RedisRpcRequest request) {
        ...
        if (channelId == null || cmdCode < 0 || parameter1 < 0 || parameter2 < 0 || combindCode2 < 0) {
            response.setStatusCode(ErrorCode.ERROR400.getCode());
            response.setBody("param error");
            return response;
        }
