package com.coalbot.module.camera.jt1078.service.impl;
...
import com.coalbot.module.camera.media.event.mediaServer.MediaSendRtpStoppedEvent;
...
public class jt1078PlayServiceImpl implements Ijt1078PlayService {
    ...
    public void onApplicationEvent(MediaArrivalEvent event) {
        if (event.getApp().equals(talkApp) && event.getStream().endsWith("_talk")) {
            ...
            sendTalk(device, channelId, event.getMediaServer(), event.getApp(), event.getStream());
    ...
    public void onApplicationEvent(MediaNotFoundEvent event) {
        ...
        String channelId = streamParamArray[2];
    ...
    private final Map<String, List<CommonCallback<RetResult<StreamInfo>>>> inviteErrorCallbackMap = new ConcurrentHashMap<>();
    ...
    private void play(JTDevice device, JTChannel channel, int type, CommonCallback<RetResult<StreamInfo>> callback) {
        ...
        List<CommonCallback<RetResult<StreamInfo>>> errorCallbacks = inviteErrorCallbackMap.computeIfAbsent(playKey, k -> new ArrayList<>());
        ...
        if (streamInfo != null) {
            ...
            if (mediaServer != null) {
                ...
                if (mediaInfo != null) {
                    ...
                    for (CommonCallback<RetResult<StreamInfo>> errorCallback : errorCallbacks) {
                        errorCallback.run(RetResponse.makeRsp(InviteErrorCode.SUCCESS.getCode(), InviteErrorCode.SUCCESS.getMsg(), streamInfo));
                    }
                    return;
                }
            }
        ...
        if (mediaServer == null) {
            for (CommonCallback<RetResult<StreamInfo>> errorCallback : errorCallbacks) {
                errorCallback.run(RetResponse.makeRsp(InviteErrorCode.FAIL.getCode(), "未找到可用的媒体节点", streamInfo));
            }
            return;
        }
        // 设置hook监听
        ...
        subscribe.addSubscribe(hook, (hookData) -> {
            ...
            for (CommonCallback<RetResult<StreamInfo>> errorCallback : errorCallbacks) {
                ...
                errorCallback.run(RetResponse.makeRsp(InviteErrorCode.SUCCESS.getCode(), InviteErrorCode.SUCCESS.getMsg(), info));
            }
        ...
        dynamicTask.startDelay(playKey, () -> {
            ...
            for (CommonCallback<RetResult<StreamInfo>> errorCallback : errorCallbacks) {
                errorCallback.run(RetResponse.makeRsp(InviteErrorCode.ERROR_FOR_STREAM_TIMEOUT.getCode(),
        ...
        j9101.setChannel(Integer.parseInt(channelId));
    ...
    public void stopPlay(String phoneNumber, String channelId) {
        ...
        dynamicTask.stop(playKey);
        // 清理回调
        List<CommonCallback<RetResult<StreamInfo>>> generalCallbacks = inviteErrorCallbackMap.get(playKey);
        if (generalCallbacks != null && !generalCallbacks.isEmpty()) {
            for (CommonCallback<RetResult<StreamInfo>> callback : generalCallbacks) {
                callback.run(RetResponse.makeRsp(InviteErrorCode.ERROR_FOR_FINISH.getCode(), InviteErrorCode.ERROR_FOR_FINISH.getMsg(), null));
            }
        }
        jt1078Template.checkTerminalStatus(phoneNumber);
        StreamInfo streamInfo = (StreamInfo) redisTemplate.opsForValue().get(playKey);
        ...
        j9102.setChannel(Integer.parseInt(channelId));
        j9102.setCommand(0);
    ...
    private void playback(JTDevice device, JTChannel channel, String startTime, String endTime, Integer type,
        ...
        List<CommonCallback<RetResult<StreamInfo>>> errorCallbacks = inviteErrorCallbackMap.computeIfAbsent(playbackKey, k -> new ArrayList<>());
        errorCallbacks.add(callback);
        ...
        dynamicTask.startDelay(playbackKey, () -> {
            ...
            for (CommonCallback<RetResult<StreamInfo>> errorCallback : errorCallbacks) {
                errorCallback.run(RetResponse.makeRsp(InviteErrorCode.ERROR_FOR_SIGNALLING_TIMEOUT.getCode(),
    ...
    @Override
    public StreamInfo startTalk(String phoneNumber, String channelId) {
        // 检查流是否已经存在，存在则返回
    ...
    private void sendTalk(JTDevice device, String channelId, MediaServer mediaServer, String app, String stream) {
        // 检查待发送的流是否存在，
...
//        sendRtpServerService.update(sendRtpInfo);
    ...
    @Override
    public void playBack(String channelId, Long startTime, Long stopTime, ErrorCallback<StreamInfo> callback) {
