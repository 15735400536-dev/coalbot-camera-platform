...
public interface DeviceMapper {
    ...
    @Select("SELECT " +
            ...
            "(SELECT count(0) FROM wcp_device_channel dc WHERE dc.data_type = 1 and dc.data_device_id= de.id) as channel_count " +
            " FROM wcp_device de WHERE de.device_id = #{deviceId}")
    Device getDeviceByDeviceId(@Param("deviceId") String deviceId);

    @Insert("INSERT INTO wcp_device (" +
            "device_id, " +
            "name, " +
            "manufacturer, " +
            "model, " +
    ...
    @Select(
            " <script>" +
                    ...
                    "on_line," +
                    "media_server_id," +
                    "(SELECT count(0) FROM wcp_device_channel dc WHERE dc.data_type = #{dataType} and dc.data_device_id= de.id) as channel_count " +
                    "FROM wcp_device de" +
    ...
    @Select("SELECT " +
            ...
            "server_id," +
            "on_line" +
            " FROM wcp_device WHERE on_line = true")
    ...
    @Select("SELECT " +
            ...
            "server_id," +
            "on_line" +
            " FROM wcp_device WHERE on_line = true and server_id = #{serverId}")
    ...
    @Select("SELECT " +
            ...
            "geo_coord_sys," +
            "on_line" +
            " FROM wcp_device WHERE ip = #{host} AND port=#{port}")
    ...
    @Select("select * FROM wcp_device")
    List<Device> getAll();

    @Select("select * FROM wcp_device where  as_message_channel = true")
    ...
    @Select("select * from wcp_device_channel where id = #{id}")
    DeviceChannel getRawChannel(@Param("id") String id);

    @Select("select * from wcp_device where id = #{id}")
    Device query(@Param("id") String id);

    @Select("select wd.* from wcp_device wd left join wcp_device_channel wdc on wdc.data_type = #{dataType} and wd.id = wdc.data_device_id  where wdc.id = #{channelId}")
    Device queryByChannelId(@Param("dataType") Integer dataType, @Param("channelId") String channelId);

    @Select("select wd.* from wcp_device wd left join wcp_device_channel wdc on wdc.data_type = #{dataType} and wd.id = wdc.data_device_id  where wdc.device_id = #{channelDeviceId}")
