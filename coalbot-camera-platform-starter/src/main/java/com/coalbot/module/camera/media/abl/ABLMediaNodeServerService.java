...
import com.coalbot.module.camera.media.abl.bean.ABLMedia;
import com.coalbot.module.camera.media.abl.bean.ABLResult;
import com.coalbot.module.camera.media.abl.bean.AblServerConfig;
...
//    }
    ...
    @Override
    public RetResult<String> addStreamProxy(MediaServer mediaServer, String app, String stream, String url, boolean enableAudio, boolean enableMp4, String rtpType, Integer timeout) {

        ABLResult result = ablresTfulUtils.addStreamProxy(mediaServer, app, stream, url, !enableAudio, enableMp4, rtpType, timeout);
        if (result.getCode() != 0) {
            return RetResponse.makeRsp(ErrorCode.ERROR100.getCode(), result.getMemo());
        }else {
            return RetResponse.makeOKRsp(result.getKey());
        }
    }
